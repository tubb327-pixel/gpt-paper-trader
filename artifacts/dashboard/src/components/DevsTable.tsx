  ...
  return (
    <table className="w-full text-[12px]">
      ...
      <tbody>
        {rows.length === 0 && (
          <tr>
            <td colSpan={5} className="text-center text-[#8b8b9a] py-8">
              No data
            </td>
          </tr>
        )}
        {rows.map((row, i) => (
          ...
          >
            <td className="px-3 py-1.5">
              <span
                className={`w-2 h-2 rounded-full inline-block ${ARCHETYPE_DOT[row.archetype]}`}
              />
            </td>
            ...
            <td className="px-3 py-1.5 text-right font-mono tabular-nums text-[#e8e8f0]">
              {row.migration_rate != null
                ? `${(row.migration_rate * 100).toFixed(0)}%`
                : "—"}
            </td>
