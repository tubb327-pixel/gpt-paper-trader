  ...
  return (
    <>
      ...
      >
        ...
        <td
          className={`px-3 py-1.5 text-right font-mono tabular-nums text-[12px] font-semibold ${
            retPos ? "text-[#00d4aa]" : "text-[#ff4757]"
          }`}
        >
          {formatPct(trade.return_pct ?? null)}
        </td>
        <td
          className={`px-3 py-1.5 text-right font-mono tabular-nums text-[12px] ${
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[11px] font-mono text-[#8b8b9a]">
              <div>
                ...
                <span className="text-[#e8e8f0]">
                  {trade.entry_at ? formatTimeCst(trade.entry_at) : "—"}
                </span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-[#00d4aa]">
                  {formatPct(trade.peak_return_pct ?? null)}
                </span>
              </div>
              <div>
                <span className="text-[#555566]">Archetype: </span>
                <span className="text-[#e8e8f0]">
                  {trade.archetype ?? "unknown"}
                </span>
              </div>
...
export function ClosedTradesFeed() {
  ...
  const grouped: { date: string; trades: ClosedTrade[] }[] = [];
      ...
      <div className="overflow-auto h-full">
        <table className="w-full text-[12px]">
          ...
          <tbody>
            {trades.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center text-[#8b8b9a] py-10">
                  No closed trades
                </td>
              </tr>
            )}
            {grouped.map(({ date, trades: rows }) => (
              <>
                <tr key={`hdr-${date}`} className="bg-[#0e0e16]">
                  <td
                    colSpan={6}
                    className="px-3 py-1 text-[10px] font-semibold tracking-widest text-[#8b8b9a] uppercase"
                  >
                    {date}
                  </td>
                </tr>
                {rows.map((t) => (
                  <FeedRow key={t.launch_id} trade={t} />
                ))}
              </>
            ))}
          </tbody>
